...
本版按照 PPT 平台约束修正为稳定同步结构：`CP2` 用于内部同步、按键采样和扬声器音调，`CP3` 作为外部秒脉冲输入并提供闹钟滴声节奏，避免 `PULSE/QD` 被综合成异步控制后在板上乱跳。扬声器蜂鸣复用 `CP2`，不再占用 `CP1` 时钟。
...
- `CP3`：运行计时基准和闹钟滴声间隔，接 PPT 中的 `10Hz/1Hz` 引脚

结合 [实验五+六-预习.pdf](/D:/clock/实验五+六-预习.pdf) 的引脚说明，上板时建议：

- 将 `DZ8` 短接到 `1Hz`，让 `CP3` 提供每秒一次计时脉冲
- 将 `CP2` 选择为 `1KHz`，用于同步 `QD/PULSE/K0/K1/K2/K3`，同时给扬声器提供约 `500Hz` 的音调；闹钟每秒短响一次，每次约 `125ms`
- `CP1` 不再作为输入使用
...
- 到达闹钟时间 `HH:MM:00` 时，扬声器每隔约 `1s` 短响一次，音调由 `CP2` 翻转得到
- 响铃期间电子钟继续正常走时
- 响铃时按 `QD` 消音
