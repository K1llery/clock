...
本版按照 PPT 平台约束修正为稳定同步结构：`CP2` 用于内部同步、按键采样和扬声器蜂鸣，`CP3` 作为外部秒脉冲输入，避免 `PULSE/QD` 被综合成异步控制后在板上乱跳。扬声器蜂鸣复用 `CP2`，不再占用 `CP1` 时钟。
...
- `CP1`：本版不使用，可不接入
- `CP2`：同步采样和扬声器蜂鸣复用时钟，接 PPT 中的 `1KHz/100Hz` 引脚；推荐选择 `1KHz`
- `CP3`：运行计时基准，接 PPT 中的 `10Hz/1Hz` 引脚

结合 [实验五+六-预习.pdf](/D:/clock/实验五+六-预习.pdf) 的引脚说明，上板时建议：

- 将 `DZ8` 短接到 `1Hz`，让 `CP3` 提供每秒一次计时脉冲
- 将 `CP2` 选择为 `1KHz`，用于同步 `QD/PULSE/K0/K1/K2/K3`，同时给扬声器提供约 `500Hz` 的方波蜂鸣；如果选 `100Hz`，声音会变成约 `50Hz`
- `CP1` 不再作为输入使用
...
- 到达闹钟时间 `HH:MM:00` 时，扬声器输出由 `CP2` 翻转得到的方波蜂鸣
- 响铃期间电子钟继续正常走时
- 响铃时按 `QD` 消音
...
- `CP2` 必须有有效时钟输入，推荐选择 `1KHz`；如果 `CP2` 选择为 `100Hz`，蜂鸣会变成较低的约 `50Hz`
